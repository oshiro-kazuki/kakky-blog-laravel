{
    ...
    let submit_flg = [];
    for(let i = 0; i < 11; ++i){
        i < 2 ? submit_flg.push(true): submit_flg.push(false);
    }
    const title_len = 20;
    const text_len = 1000;
    let count = 0;
    ...
    window.addEventListener('load', () => {
        ...
        nullableText('origin_title', 'origin_title_err', 'origin_title_conf', title_len, submit_flg, ++count, 'form_conf_btn', 'submit_btn');
        nullableText('origin_text', 'origin_text_err', 'origin_text_conf', text_len, submit_flg, ++count, 'form_conf_btn', 'submit_btn');
